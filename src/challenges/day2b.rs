...
impl Round {
    ...
    fn to_normal_round(&self) -> Round{
        let player_hand = match (self.player_hand, self.op_hand) {

            (decider, op) => match decider {
                Hand::Paper => op, // draw
                Hand::Scissors => match op { // win
                    Hand::Paper => Hand::Scissors, 
                    Hand::Scissors => Hand::Rock, 
                    Hand::Rock => Hand::Paper, 
                },
                Hand::Rock => match op { // lose
    ...
    fn calculate(&self) -> i32{
        ...
        let new_round = self.to_normal_round();
      
        let player_value = match new_round.player_hand {
            Hand::Rock => 1,
            Hand::Paper => 2,
            Hand::Scissors => 3,
        };

        let round_value = match (&new_round.player_hand, &new_round.op_hand) {
